#!/usr/bin/env python3
"""Build checked static learned-membership proof for EMei from pinned PC evidence."""
...
def progression_by_tier(path: Path) -> tuple[dict[int, list[int]], dict[int, int]]:
    ...
    start = next(i for i, line in enumerate(lines) if re.match(rb"^\s*emei\s*=\s*\{\s*$", line))
...
def display_ids(panel: Path) -> list[int]:
    match = re.search(r"public static readonly int\[\] PcEMeiSkillOrder\s*=\s*\{(.*?)\};", panel.read_text(encoding="utf-8"), re.S)
    if not match:
        raise SystemExit("missing PcEMeiSkillOrder")
...
def build(source: Path, provenance: Path, target_source: Path, target_provenance: Path,
    ...
    if len(learned) != 21 or len(display) != 15 or len(union) != 25:
        raise SystemExit("unexpected EMei membership counts")
    verify_manifest(provenance, union, source)
    table = rows(source)
    if set(table) != set(union):
        raise SystemExit("EMei union slice rows drift")
    targets, by_skill = relationships(table, learned)
    if len(targets) != 23:
        raise SystemExit("unexpected EMei relationship closure")
    ...
    if set(rows(target_source)) != set(targets):
        raise SystemExit("EMei relationship target slice rows drift")
    ...
    membership_path = ROOT / "harness/docs/stories/SKL-EM-PROOF-001/membership-classification.json"
    proof = {
...
def main() -> int:
    ...
    parser.add_argument("--source", type=Path, default=story / "PcEMeiSkills.txt")
    parser.add_argument("--provenance", type=Path, default=story / "PcEMeiSkills.provenance.json")
    parser.add_argument("--target-source", type=Path, default=story / "PcEMeiRelationshipTargets.txt")
    parser.add_argument("--target-provenance", type=Path, default=story / "PcEMeiRelationshipTargets.provenance.json")
    ...
    union = sorted({77, 79, 80, 81, 82, 83, 84, 85, 86, 87, 88, 89, 91, 92, 93, 252, 282, 328, 332, 380, 385, 712, 1061, 1062, 1114})
    ...
    if args.check:
        if not args.membership.is_file() or not args.proof.is_file() or args.membership.read_bytes() != membership or args.proof.read_bytes() != proof:
            raise SystemExit("EMei proof artifact drift")
    else:
