#!/usr/bin/env python3
"""Build checked static learned-membership proof for WuDu from pinned PC evidence."""
...
def progression_by_tier(path: Path) -> tuple[dict[int, list[int]], dict[int, int]]:
    ...
    start = next(i for i, line in enumerate(lines) if re.match(rb"^\s*wudu\s*=\s*\{\s*$", line))
...
def display_ids(panel: Path) -> list[int]:
    match = re.search(r"public static readonly int\[\] PcWuDuSkillOrder\s*=\s*\{(.*?)\};", panel.read_text(encoding="utf-8"), re.S)
    if not match:
        raise SystemExit("missing PcWuDuSkillOrder")
...
def build(source: Path, provenance: Path, target_source: Path, target_provenance: Path,
    ...
    if len(learned) != 23 or len(display) != 16 or len(union) != 24:
        raise SystemExit("unexpected WuDu membership counts")
    verify_manifest(provenance, union, source)
    table = rows(source)
    if set(table) != set(union):
        raise SystemExit("WuDu union slice rows drift")
    targets, by_skill = relationships(table, learned)
    if len(targets) != 16:
        raise SystemExit("unexpected WuDu relationship closure")
    ...
    if set(rows(target_source)) != set(targets):
        raise SystemExit("WuDu relationship target slice rows drift")
    ...
    membership_path = ROOT / "harness/docs/stories/SKL-WDU-PROOF-001/membership-classification.json"
    proof = {
...
def main() -> int:
    ...
    parser.add_argument("--source", type=Path, default=story / "PcWuDuSkills.txt")
    parser.add_argument("--provenance", type=Path, default=story / "PcWuDuSkills.provenance.json")
    parser.add_argument("--target-source", type=Path, default=story / "PcWuDuRelationshipTargets.txt")
    parser.add_argument("--target-provenance", type=Path, default=story / "PcWuDuRelationshipTargets.provenance.json")
    ...
    union = sorted({60, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 353, 355, 356, 384, 390, 711, 1066, 1067})
    ...
    if args.check:
        if not args.membership.is_file() or not args.proof.is_file() or args.membership.read_bytes() != membership or args.proof.read_bytes() != proof:
            raise SystemExit("WuDu proof artifact drift")
    else:
