#!/usr/bin/env python3
"""Build checked static learned-membership proof for TianRen from pinned PC evidence."""
...
def progression_by_tier(path: Path) -> tuple[dict[int, list[int]], dict[int, int]]:
    ...
    start = next(i for i, line in enumerate(lines) if re.match(rb"^\s*tianren\s*=\s*\{\s*$", line))
...
def display_ids(panel: Path) -> list[int]:
    match = re.search(r"public static readonly int\[\] PcTianRenSkillOrder\s*=\s*\{(.*?)\};", panel.read_text(encoding="utf-8"), re.S)
    if not match:
        raise SystemExit("missing PcTianRenSkillOrder")
...
def build(source: Path, provenance: Path, target_source: Path, target_provenance: Path,
    ...
    if len(learned) != 20 or len(display) != 18 or len(union) != 25:
        raise SystemExit("unexpected TianRen membership counts")
    verify_manifest(provenance, union, source)
    table = rows(source)
    if set(table) != set(union):
        raise SystemExit("TianRen union slice rows drift")
    targets, by_skill = relationships(table, learned)
    if len(targets) != 15:
        raise SystemExit("unexpected TianRen relationship closure")
    ...
    if set(rows(target_source)) != set(targets):
        raise SystemExit("TianRen relationship target slice rows drift")
    ...
    membership_path = ROOT / "harness/docs/stories/SKL-TR-PROOF-001/membership-classification.json"
    proof = {
...
def main() -> int:
    ...
    parser.add_argument("--source", type=Path, default=story / "PcTianRenSkills.txt")
    parser.add_argument("--provenance", type=Path, default=story / "PcTianRenSkills.provenance.json")
    parser.add_argument("--target-source", type=Path, default=story / "PcTianRenRelationshipTargets.txt")
    parser.add_argument("--target-provenance", type=Path, default=story / "PcTianRenRelationshipTargets.provenance.json")
    ...
    union = sorted({131, 132, 135, 136, 137, 138, 139, 140, 141, 142, 143, 144, 145, 146, 147, 148, 149, 150, 361, 362, 364, 391, 715, 1075, 1076})
    ...
    if args.check:
        if not args.membership.is_file() or not args.proof.is_file() or args.membership.read_bytes() != membership or args.proof.read_bytes() != proof:
            raise SystemExit("TianRen proof artifact drift")
    else:
