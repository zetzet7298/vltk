#!/usr/bin/env python3
"""Build checked static learned-membership proof for WuDang from pinned PC evidence."""
...
def progression_by_tier(path: Path) -> tuple[dict[int, list[int]], dict[int, int]]:
    ...
    start = next(i for i, line in enumerate(lines) if re.match(rb"^\s*wudang\s*=\s*\{\s*$", line))
...
def display_ids(panel: Path) -> list[int]:
    match = re.search(r"public static readonly int\[\] PcWuDangSkillOrder\s*=\s*\{(.*?)\};", panel.read_text(encoding="utf-8"), re.S)
    if not match:
        raise SystemExit("missing PcWuDangSkillOrder")
...
def build(source: Path, provenance: Path, target_source: Path, target_provenance: Path,
    ...
    if len(learned) != 17 or len(display) != 16 or len(union) != 22:
        raise SystemExit("unexpected WuDang membership counts")
    verify_manifest(provenance, union, source)
    table = rows(source)
    if set(table) != set(union):
        raise SystemExit("WuDang union slice rows drift")
    targets, by_skill = relationships(table, learned)
    if len(targets) != 15:
        raise SystemExit("unexpected WuDang relationship closure")
    ...
    if set(rows(target_source)) != set(targets):
        raise SystemExit("WuDang relationship target slice rows drift")
    ...
    membership_path = ROOT / "harness/docs/stories/SKL-WD-PROOF-001/membership-classification.json"
    proof = {
...
def main() -> int:
    ...
    parser.add_argument("--source", type=Path, default=story / "PcWuDangSkills.txt")
    parser.add_argument("--provenance", type=Path, default=story / "PcWuDangSkills.provenance.json")
    parser.add_argument("--target-source", type=Path, default=story / "PcWuDangRelationshipTargets.txt")
    parser.add_argument("--target-provenance", type=Path, default=story / "PcWuDangRelationshipTargets.provenance.json")
    ...
    union = sorted({151, 152, 153, 154, 155, 156, 157, 158, 159, 160, 161, 162, 163, 164, 165, 166, 267, 365, 368, 716, 1078, 1079})
    ...
    if args.check:
        if not args.membership.is_file() or not args.proof.is_file() or args.membership.read_bytes() != membership or args.proof.read_bytes() != proof:
            raise SystemExit("WuDang proof artifact drift")
    else:
