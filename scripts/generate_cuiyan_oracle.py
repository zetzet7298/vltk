#!/usr/bin/env python3
"""Build checked static learned-membership proof for CuiYan from pinned PC evidence."""
...
def progression_by_tier(path: Path) -> tuple[dict[int, list[int]], dict[int, int]]:
    ...
    start = next(i for i, line in enumerate(lines) if re.match(rb"^\s*cuiyan\s*=\s*\{\s*$", line))
...
def display_ids(panel: Path) -> list[int]:
    match = re.search(r"public static readonly int\[\] PcCuiYanSkillOrder\s*=\s*\{(.*?)\};", panel.read_text(encoding="utf-8"), re.S)
    if not match:
        raise SystemExit("missing PcCuiYanSkillOrder")
...
def build(source: Path, provenance: Path, target_source: Path, target_provenance: Path,
    ...
    if len(learned) != 17 or len(display) != 13 or len(union) != 19:
        raise SystemExit("unexpected CuiYan membership counts")
    verify_manifest(provenance, union, source)
    table = rows(source)
    if set(table) != set(union):
        raise SystemExit("CuiYan union slice rows drift")
    targets, by_skill = relationships(table, learned)
    if len(targets) != 17:
        raise SystemExit("unexpected CuiYan relationship closure")
    ...
    if set(rows(target_source)) != set(targets):
        raise SystemExit("CuiYan relationship target slice rows drift")
    ...
    membership_path = ROOT / "harness/docs/stories/SKL-CY-PROOF-001/membership-classification.json"
    proof = {
...
def main() -> int:
    ...
    parser.add_argument("--source", type=Path, default=story / "PcCuiYanSkills.txt")
    parser.add_argument("--provenance", type=Path, default=story / "PcCuiYanSkills.provenance.json")
    parser.add_argument("--target-source", type=Path, default=story / "PcCuiYanRelationshipTargets.txt")
    parser.add_argument("--target-provenance", type=Path, default=story / "PcCuiYanRelationshipTargets.provenance.json")
    ...
    union = sorted({95, 97, 99, 100, 101, 102, 103, 105, 108, 109, 111, 113, 114, 269, 336, 337, 713, 1063, 1065})
    ...
    if args.check:
        if not args.membership.is_file() or not args.proof.is_file() or args.membership.read_bytes() != membership or args.proof.read_bytes() != proof:
            raise SystemExit("CuiYan proof artifact drift")
    else:
