...
After runtime QA, mount/dismount worked but Khinh Công, walk/run, and meditate were still visually no-op or functionally gated. Additional PC-source research found the missing player action suffixes in `jx-pc` `Client 6.0/settings/npcres/男主角未骑马关联表.txt` and `男主角躯体.txt`:
