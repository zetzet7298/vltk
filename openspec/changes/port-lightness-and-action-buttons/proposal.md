...
- Port PC Khinh Công (lightness skill) from `vltksource_new`, including its PC icon, and assign it to the slot made empty by the swap.
...
| PC icon hash | JX Pack hash `gbk=bf787a8a`, found at `/var/www/vltksource_new/vl_update_27/pak_unpacked/update01/unknown/bf787a8a.spr` and `/var/www/vltksource_new/vl_update_27/pak_unpacked/spr/unknown/bf787a8a.spr`. |
...
The mandatory PC source path from the porting rule, `/var/www/vltksource_new/01_tinh_kiem_source/source/00.src-tinh-kiem`, is absent on disk in this environment. Implementation must therefore treat `/var/www/vltk-mobile/Assets/StreamingAssets/Reference/PcSkills.txt` plus `/var/www/vltksource_new/vl_update_27/pak_unpacked/...` as the available PC evidence for this proposal, and should record that caveat in code/provenance where PC runtime behavior cannot be fully verified.
