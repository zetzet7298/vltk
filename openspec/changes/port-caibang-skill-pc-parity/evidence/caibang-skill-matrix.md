...
| 127 | Hoạt Bất Lưu Thủ | `huabu_liushou` | Current service maps 127 (verified); `CaiBangCombatParityTests` asserts `FastWalkRunP=66,3240,0` at L20; skill style tests assert `PassivityNpcState` and state special 17. | needs-runtime-audit | Prove movement runtime actually consumes `FastWalkRunP`; verify expiration. |
| 128 | Kháng Long Hữu Hối | `kanglong_youhui` | Current service maps 128; visual service currently calls `GetMissileForm` and routes `missileForm == 2` to `SetupPcKangLongSpread`. | verified-current for branch presence; needs-visual-audit | Add/verify tests for fan spread from `skill_misslesform_v` and `skill_param1_v`. |
| 130 | Túy Điệp Cuồng Vũ | `zuidie_kuangwu` | Current service maps 130 (verified); char anim test asserts `charAnimId=43`; combat slot test references visual aura. | needs-runtime-audit | Verify allres/fire/deadly/life/cost values apply and expire. |
| 357 | Phi Long Tại Thiên | `feilong_zaitian`; skill row child missile `166`; missile `166` has `MoveKind=5` homing | Current service maps 357; tests assert L20 missile count 4; visual service has live target accessor and per-missile `ResolveMissileTarget`; `CaiBangCombatParityTests` has homing offset test. Resource evidence recorded in `phi-long-resource-evidence.md`. | verified-current for key code/resource evidence; needs visual smoke | Resolve ignored homing-offset test caveat around missing `skill_param1_v`; visually verify L20 four-dragon homing with PC SPRs. |
