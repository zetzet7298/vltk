...
| 125 | Bổng Đả Ác Cẩu | `bangda_egou` | Newest PC row 125 maps to `bangda_egou`, not `tianxia_wugou`. Runtime supports both PC chains: `addskilldamage1 -> 359` chance 60 and `addskilldamage2 -> 1074` chance 50. Deterministic cast test expects 8 chain projectiles (3+5). Evidence in `active-chain-version-priority-evidence.md`. | verified-current for mapping/chain/test evidence | Visual smoke still needed. |
...
| 359 | Thiên Hạ Vô Cẩu / MOD variant | `tianxia_wugou` | Current service maps 359; tests assert missile 168 speed/lifetime and L20 count 3; addskilldamage chain to 1074 chance 25. Evidence in `active-chain-version-priority-evidence.md`. | verified-current for key runtime/chain values | Verify visual with PC SPR. |
