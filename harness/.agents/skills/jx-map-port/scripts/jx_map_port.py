...
def main():
    ...
    data_dir = args.data_dir or os.path.join(args.source_root, "bin/Client/data")
    run_maps = os.path.join(args.source_root, "Utility/Run/maps")
    art_roots = [os.path.join(args.source_root, "Utility/Run"),
                 os.path.join(args.source_root, "bin/Client")]
    ...
    if not os.path.isdir(data_dir):
        print(f"ERROR: pak dir not found: {data_dir}; pass --data-dir with canonical PAK input", file=sys.stderr)
        return 2
