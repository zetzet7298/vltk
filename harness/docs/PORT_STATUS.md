...
| 12.2 | HUD Art (PC SPR) | 1,851 SPR | ~410 | 🔄 | 2026-06-08 HUD parity pass theo user reference Image #1 / `pc-evidence/pc_hud.png`: bỏ stale 1024/jx1024 layout cho HUD đang render; top bar dùng PC 800 `顶部控制条` uid `8da7027d` + `top_status_strip.png` uid `973816f3` (552×17), fill SPR `74b299b9/83e13762/b72be14b/f5d017dd` (106×11). Bottom/action/menu dùng PC 800 source `slistcache.pak` `主界面玩家信息窗口` uid `e3b06434` + `工具控制条` uid `dc11ac12`; `bottom_bar_bg.png` là composite 1280×106 từ PC 800 pixels, left fragment giữ hotbar/T-P, right fragment action/menu/Bảo Vật right-anchored, không stretch icon. `HudBottomBarPcSpec` chuyển sang tọa độ 800 (`Items 491,559`, `Run 567,502`, `Treasure 742,502`) và `GameHudController` tạo proxy hitbox cho mọi nút PC để không lệch icon/click. Tests: `HudBottomBarAuthenticityTests` khóa size/pixel fragments/spec. |
