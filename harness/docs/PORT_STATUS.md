...
| 12.2 | HUD Art (PC SPR) | 1,851 SPR | ~410 | 🔄 | StreamingAssets/UI/HUD/Art/ có ~410 PNG + HudArtCatalogService runtime; HUD user-facing baked Chinese audit có 279 PNG CJK, catalog Việt hóa + tests chặn close/refresh/chat/skill/friend/guild/team text Trung. Bottom bar = SPR PC thật `jx1024.spr` (uid 917565dd, 1024.pak) band y[680-768]=1024x89, KHÔNG crop screenshot; spec layout từ INI `工具控制条` (dc11ac12)+`主界面玩家信息窗口` (e3b06434) → HudBottomBarPcSpec.cs + HudBottomBarAuthenticityTests.cs. Top bar: bars repositioned tới toạ độ PC-pixel (stamina/hp/mp/exp 289/465/641/816), khung phẳng PC-style, level/rank box, connection text — khớp pixel pc_hud.png. Minimap: viền mỏng 1px (đúng PC, không ornate), 2 nút tiện ích icon SPR thật. Bottom bar menu icons = SPR PC gốc từ 1024.pak (b5b2ef55/3d42308f/56d016e0/04e4c9e4/823193a1/46c617c6/00a58516/1023e503) + Bảo Vật cam thật (8f845112, updatejx08.pak) composite. **Round-action buttons (2026-06-07): SPR nút run/sit/horse/exchange CHỨNG MINH không tồn tại trong bất kỳ PAK/manifest nào (hash `file_id_from_bytes` verified), nên pixel authentic crop trực tiếp từ pc_hud.png (btn_run/sit/horse/exchange.png 30×30, alpha tròn) — nguồn PC thật duy nhất, không bịa. Root cause icon ẩn: `#BottomPanel .hud-btn-icon{display:none}` (line 748) che mọi icon toolbar → fix `#BottomPanel .hud-round-btn .hud-btn-icon{display:flex}` (line 751). Verified live UI tree (execute_code UQuery): 4 nút tròn @panel x=324/353/383/412 y=665 28×28 display=Flex visible, icon child bg=TEX 32×32. Region diff vs pc_hud.png PASS: bottom-right toolbar (nút tròn+cụm menu+orb Bảo Vật vàng), top status bar (HP/MP/EXP/stamina fill clip-by-percent đúng — 0% EXP rỗng cả 2), full-frame structure faithful.** |
| 12.3 | Vietnamese Text Overlay | - | ✅ | ✅ | PcHudVietnameseTextOverlay |
| 12.4 | Skill Panel | Yes | ✅ | ✅ | PcSkillPanelService + CombatSkillSlotController. Runtime 2 ô skill T/P: chạm ô trống → picker chọn skill (lọc kỹ năng chủ động đã học của môn phái), chạm-giữ 450ms → mở lại picker để đổi skill, chạm ô đã gán → auto-target enemy gần nhất + cast (CombatRuntime, port 100% từ PC). Icon skill gán hiển thị đè lên art PC nhờ inline display override (USS `#BottomPanel` ẩn icon ô trống để lộ art baked). Verified play mode: T cast Ném Đá Hỏi Đường → Hươu đốm dist=151, dmg applied 100→0. **Fix touch (2026-06-07): ScrollView nuốt ClickEvent của item khi chạm trên mobile → tap chọn skill trong picker không gán được; chuyển sang phát hiện tap ở cấp ScrollView (record press pos + move-threshold 12px để không xung đột scroll-drag) rồi hit-test item dưới điểm thả (AssignSkillUnderPoint + item.userData=skillId). Verified real PointerDown→PointerUp: tap item gán skill (L→121) + đóng picker; tap ô trống mở picker; long-press 450ms ô đã gán mở lại picker** **Fix layout (2026-06-07): 2 ô T/P bị lệch trái (đè hotbar 4-5) và ToolbarRight (nút Kỹ năng) sibling sau → tap giữa trúng nút menu mở bảng khác. Sửa: hud-skill-panel đặt `position:absolute left612 top19` đè đúng art T/P trong khung ngọc (bottom_bar_bg.png 1280×82: T x[613-657], P x[660-705]) + `BringToFront()` để paint trên ToolbarRight. Verified: panel.Pick tại tâm T/P trả đúng SlotIcon (không còn trúng menu); pixel-diff empty-vs-assigned T 2313/3969 P 2265/4158 (maxdelta~678) → icon render đúng vị trí giữa khung** **Fix long-press (2026-06-07): trên touch thật ngón rung sinh `PointerCancelEvent` (mất capture) giết press trước 450ms → chạm-giữ không mở lại danh sách switch skill. Sửa: cancel chỉ release capture, GIỮ press sống (không reset state) → timer long-press vẫn mở picker; OnSlotMove chỉ hủy khi kéo xa >45px (drag thật), jitter nhỏ bỏ qua. Verified: Down→Cancel→đợi 450ms vẫn mở picker (PASS); tap nhanh ô đã gán vẫn ra đòn không mở picker (PASS)** **Fix paint-order regress (2026-06-07): root cause thật của "tap T mở bảng Kỹ năng / long-press không hiện": GameHudController rebuild children của BottomPanel SAU khi bind → đẩy hud-skill-panel về index 1 (trước ToolbarRight/BtnTreasure) → ToolbarRight (wb x[324..687] trùm T/P) và BtnTreasure nuốt toàn bộ touch tại T/P → slot handler không bao giờ chạy. One-shot BringToFront() không stick. Sửa: re-assert `skillPanel.BringToFront()` qua `GeometryChangedEvent` của parent (mỗi lần layout đổi). Verified cold-start: panel.Pick tâm T=SlotIcon R=SlotIcon (KHÔNG còn trúng ToolbarRight/BtnTreasure), menu vẫn pick (BtnSkills=BtnSkills); chuỗi long-press đầy đủ log `[LP] BeginSlotPress` + `timer fired down=True` → picker mở và GIỮ mở sau khi nhả ngón (PASS)** **Fix TRIỆT ĐỂ (2026-06-07, research exa+Unity docs): research xác nhận Unity route pointer events theo VisualElement HIERARCHY chứ KHÔNG theo draw order — "layered objects click-through khi là con của nhau, KHÔNG khi là sibling"; và "BringToFront bị ghi đè khi add child mới". Nên BringToFront/GeometryChanged re-assert KHÔNG đáng tin (sibling reorder sau rebuild → ToolbarRight/BtnTreasure thành pick-target nuốt tap). Giải pháp chuẩn Unity: chặn `PointerDownEvent` ở ROOT, TrickleDown phase, hit-test bounds T/P thủ công → `BeginSlotPress` + `StopImmediatePropagation`. Pointer capture lo move/up/cancel. Verified worst-case (ép SendToBack panel → menu đè T/P): pick@T=ToolbarRight pick@P=BtnTreasure nhưng PointerDown vẫn `_slotPointerDown=True _pressedSlot=0` (root intercept WORKS even menu on top); long-press mở picker dù menu đè (PASS); menu buttons NGOÀI bounds T/P (BtnSkills x507, BtnRun x324) không bị root intercept ăn → bấm bình thường (PASS). MIỄN NHIỄM sibling-order rebuild — fix tận gốc.** |
| 12.5 | Minimap Panel | Yes | ✅ | ✅ | MinimapPanel |
| 12.6 | Quest Tracker Panel | Yes | ✅ | ✅ | QuestTrackerPanel |
| 12.7 | Inventory Panel | Yes | ✅ | ✅ | **HUD bag window "Hành Trang" (2026-06-07)**: nút Túi đồ (BtnItems) bottom bar → `GameHudController.OnItemsClick()` → `ToggleInventory()` mở/đóng cửa sổ overlay (port hành vi PC: `背包按钮` ClassType `Player_Items` + phím F4/U → `Open([[items]])` từ 1024.pak/autoexec.lua). UI dựng từ INI gốc 1024.pak: layout `6a5d8c4c.ini` (储物箱 grid window — `[Main]` 214×454, `[ItemBox]` UnitBorder=2, `[Money]` Color=255,217,78, `[Settings]` BGBorderColor=100,80,30), màu phẩm chất `7bfc9072.ini` (White 255,255,255 / Blue 51,102,250 / Purple 188,64,255 / Gold 243,194,90 / Red 255,51,51). **SPR khung (储物箱更新.spr) CHỨNG MINH không tồn tại trong bất kỳ PAK active nào (index scan 30 PAK, validated vs uid jx1024) + pc_hud.png không hiện cửa sổ mở → không có pixel art authentic → dựng khung bằng giá trị màu INI thật, KHÔNG bịa SPR.** Grid mobile 4 cột × 7 hàng = 28 ô (approved). Bind dữ liệu thật: `InventoryPanelService.BuildGridSnapshot(InventoryService, playerId, gold, silver)` đọc `SandboxManager.InventoryService.Inventory`, `ResolveQuality(ItemDefinition)` map tier (setId>0→gold, refine≥7→purple, refine≥1→blue, else white). Files: InventoryWindowPcSpec.cs (source-of-truth INI), GameHud.uxml (InventoryWindow overlay), GameHud.uss (.hud-inv-*), InventoryPanelService.cs (BuildGridSnapshot+ResolveQuality), SandboxManager.cs (InventoryService getter), GameHudController.cs (toggle/open/close/populate + tap-outside-to-close), InventoryWindowTests.cs. **Verified play-mode (execute_code): IsInventoryVisible False→Open→True (28 ô) →Close→False; screenshot xác nhận khung viền vàng + grid 4 cột + nút close đỏ render đè game world. Snapshot logic verified vs live InventoryService: slot0 Đao Gỗ q=0, slot1 Giáp Tinh Luyện(refine3) q=1, slot2 Nhẫn Bộ(setId5,cnt2) q=3, 28 slot 3 used.** ⚠️ Test assembly VLTK.Tests.EditMode bị gate sau `VLTK_ENABLE_TESTS` (OFF trong committed ProjectSettings) + có 96 lỗi compile PRE-EXISTING ở file test khác (BattleType.SongJin, CaveListFullService, ShopConfigService.GetShop...) — KHÔNG liên quan inventory; code inventory compile sạch 0 lỗi (verified qua AssemblyBuilder pipeline). |
