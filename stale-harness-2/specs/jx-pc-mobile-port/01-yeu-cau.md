...
| 4 | Backend & Data | Sở hữu Python FastAPI modular monolith, PostgreSQL, REST/realtime và transaction. |
...
| 44 | REC muốn mọi content phát hành có thể truy nguyên nguồn, phiên bản và không tạo hai nguồn gameplay authority | NFR-CONT-001: Pin version/hash/locale/nguồn; config gameplay authoritative ở Python backend, Unity chỉ nhận projection presentation/prediction; production không hot-reload | REC | Content | Backend, Unity, QA | MinIO | Tự động | Tương thích |
...
| Backend Python | Auth/bootstrap, realtime, inventory/economy và checkpoint commit đúng transaction | FR-AUTH-002, FR-SESS-001, FR-INV-002, NFR-REL-001 | TEST-CONTRACT-001, TEST-ECON-001, TEST-CHECKPOINT-001 |
...
| 1 | Backend | Python 3 + FastAPI modular monolith trong `backend/`, một deployment/realm ban đầu | Target architecture; ADR-0008 |
