...
- Production chỉ có Python FastAPI backend trong `backend/` và content resolver chuẩn; cấm fallback mock/legacy khi connect hoặc resolve thất bại.
...
| `M1` | Gameplay-first P0: Python 18 Hz combat/skill + Unity target/skill presentation và test adapter qua DevHarness | Logic/replay G3 đủ evidence; C# không có authoritative rule |
| `M2` | Python auth/bootstrap/realtime seam tối thiểu + Unity production adapter để đưa đúng gameplay M1 lên realm | REST per-action hiện tại phải được đánh giá/retire theo evidence; không còn combat REST per-tick production |
