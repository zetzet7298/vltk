...
| Ref nguồn | Điều kiện hoàn tất | Artifact/chứng minh | Trạng thái |
| --- | --- | --- | --- |
| Dòng 81-83 | Go 1.26 modular monolith, process/realm, PG16, REST+WSS; no fallback | ADR/server shard/contracts | ĐẠT ở spec |
| Quyết định user | PostgreSQL 16 dùng stack `/var/www/tt-docker`, không chép secret vào spec/client | ADR-0002, release plan và PostgreSQL runtime audit disposable | ĐẠT ở spec; deployment G6 chưa chạy |
| Dòng 85-87 | OpenAPI/Proto/content/error/idempotency/SQL; ACK sau commit | `contracts/`, realtime semantics và `contracts/sql/game.v1.negative.sql` | ĐẠT ở contract; runtime Go/ACK chưa triển khai |
