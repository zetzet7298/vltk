...
2. Wait until compilation and domain reload are complete.
3. Confirm the Editor reports ready for tools.
4. Read current errors and warnings before entering Play Mode.
5. Inspect the active scene and locate the player, target actors, combat HUD, and relevant controllers.

If Unity MCP is unavailable, report that runtime verification is blocked. CLI/static tests are not a substitute for an observed combat-slot cast.

## Development test policy

Never run the full EditMode suite in the inner loop.

Use the faction category:

```python
unityMCP___run_tests(
    mode="EditMode",
    category_names=["CaiBang"],
)
```

Or use a narrow namespace when no category exists:
...
- Add `Category("Slow")` to sprite decode or other slow visual fixtures.
- Use `TestCatalogCache.NoviceAndCaiBang` only when tests do not mutate the catalog.
- Use a fresh `PcCombatCatalogFactory` catalog for mutation tests.
- Run the repo-required full suite before push when shared combat code, asmdefs, or project policy requires it.
