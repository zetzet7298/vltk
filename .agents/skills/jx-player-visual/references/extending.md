...
1. **Catalog**:
   - Nhân vật Nam: [MalePlayerSpriteCatalog.cs](file:///var/www/vltk-mobile/Assets/Scripts/Sandbox/MalePlayerSpriteCatalog.cs)
   - Nhân vật Nữ: [FemalePlayerSpriteCatalog.cs](file:///var/www/vltk-mobile/Assets/Scripts/Sandbox/FemalePlayerSpriteCatalog.cs)
   - Cả hai Catalog đều dùng chung bảng chỉ số thứ tự vẽ (draw-order offset) cho 8 hướng.
2. **Visual Renderer**:
   - Lớp [MalePlayerVisual.cs](file:///var/www/vltk-mobile/Assets/Scripts/Sandbox/MalePlayerVisual.cs) hiển thị cho nam, hỗ trợ cưỡi ngựa và trang bị mặc định là "áo vải thô" (`BD` variant `019`) và "mũ bố cân" (`HD`/`HR`/`HT` variant `019`).
   - Lớp [FemalePlayerVisual.cs](file:///var/www/vltk-mobile/Assets/Scripts/Sandbox/FemalePlayerVisual.cs) hiển thị cho nữ, hỗ trợ tải các bộ phận với tiền tố `WO_` từ thư mục `woman`.
