// -----------------------------------------------------------------------------
// VLTK Mobile — PC armor.txt data-driven port
// Source: server/settings/item/004/armor.txt (GB2312, 46 tab-separated columns)
// Purpose: keep the PC armor catalog visible to mobile runtime by id/name/icon
// and stat deltas (base + refine). No Vietnamese translation is invented;
// nameRaw stays as the PC (Vietnamese) text and nameNormalized defaults to it.
// -----------------------------------------------------------------------------
    ...
    public static class PcArmorParser
    {
        ...
        public static ItemDefinition ParseRow(string[] cols, int itemIdHint = 0)
        {
            ...
            if (PcItemCommon.ContainsReplacementChar(nameRaw))
                item.warnings.Add($"Armor row id={itemId} contains Unicode replacement char");
            return item;
        }
    }
}
