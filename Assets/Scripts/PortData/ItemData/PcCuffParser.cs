// -----------------------------------------------------------------------------
// VLTK Mobile — PC cuff.txt data-driven port
// Source: server/settings/item/004/cuff.txt (GB2312, 46 tab-separated columns)
// Purpose: keep the PC cuff (腕 / bao tay) catalog visible to mobile runtime by
    ...
    public static class PcCuffParser
    {
        ...
        public static ItemDefinition ParseRow(string[] cols, int itemIdHint = 0)
        {
            ...
            if (PcItemCommon.ContainsReplacementChar(nameRaw))
                item.warnings.Add($"Cuff row id={itemId} contains Unicode replacement char");
            return item;
        }
    }
}
