// -----------------------------------------------------------------------------
// VLTK Mobile — PC shipin.txt data-driven port
// Source: server/settings/item/004/shipin.txt (46 tab-separated columns)
// Purpose: keep the PC trinket/shipin catalog visible to mobile runtime by
// id/name/icon, detail type, particular type, and stat deltas.
// -----------------------------------------------------------------------------
...
{
    public static class PcShipinItemParser
    {
        public const int MinColumns = 46;
        public const int StatCount = 7;
        public const int ReqCount = 6;
        public const string EvidenceNote = "pc_item_004_shipin";
        ...
        public static ItemDefinition ParseRow(string[] cols, int itemIdHint = 0)
        {
            ...
            if (PcItemCommon.ContainsReplacementChar(nameRaw))
                item.warnings.Add($"Shipin row id={itemId} contains Unicode replacement char");
            return item;
        }
    }
}
