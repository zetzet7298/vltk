...
namespace VLTK.Sandbox
{
    ...
    public sealed class TongMapEntranceRuntimeService
    {
        ...
        /// </summary>
        public TongMapEnterHostDecision CanPlayerEnter(int mapId, string player, int level, int tongId, long now)
        {
            if (_host == null) return new TongMapEnterHostDecision(false, "NoHost");

            // Map công cộng (owner = 0) hoặc map không phải Tong map.
            int owner = _host.GetTongOwner(mapId);
            if (owner == 0) return new TongMapEnterHostDecision(true, "PublicMap");

            // Owner tong → được vào.
            if (owner == tongId && _host.IsPlayerInTong(player, tongId))
                return new TongMapEnterHostDecision(true, "Owner");

            // Banned (PC TONG_GetTongMapBan).
            if (_host.IsTongBanned(tongId, mapId))
                return new TongMapEnterHostDecision(false, "Banned");

            // Expired (PC tongmap_check_expire + tongmap_get_expire_date).
            long expire = _host.GetTongExpireTime(tongId, mapId);
            if (expire > 0 && now > expire)
                return new TongMapEnterHostDecision(false, "Expired");

            // Level gate (PC tong_mix.lua: minimum level 10 to enter Tong map).
            if (!_host.CanEnterTongMap(mapId, level, tongId))
                return new TongMapEnterHostDecision(false, "LevelTooLow");

            return new TongMapEnterHostDecision(true, "Allowed");
        }
