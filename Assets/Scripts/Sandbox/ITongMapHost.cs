...
namespace VLTK.Sandbox
{
    ...
    /// <summary>
    /// Quyết định nhập cảnh Tong map (host-driven view, distinct from the
    /// plan-mode enum in TongMapEnterPlanService). PC source: faction_map.txt
    /// 33 rows + script/tong/addtongnpc.lua + tong_mix.lua level-10 gate.
    /// </summary>
    public readonly struct TongMapEnterHostDecision
    {
        public readonly bool Allowed;
        public readonly string ReasonVi;

        public TongMapEnterHostDecision(bool allowed, string reasonVi)
        {
            Allowed = allowed;
