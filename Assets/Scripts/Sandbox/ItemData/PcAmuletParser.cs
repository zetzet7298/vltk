// -----------------------------------------------------------------------------
// VLTK Mobile — PC amulet.txt data-driven port
// Source: server/settings/item/004/amulet.txt (GB2312, 46 tab-separated columns)
// Purpose: keep the PC amulet (bùa) catalog visible to mobile runtime by
// id/name/icon and stat deltas (base + refine).
// -----------------------------------------------------------------------------
...
{
    public static class PcAmuletParser
    {
        public const int MinColumns = 46;
        public const int StatCount = 7;
        public const int ReqCount = 6;
        public const string EvidenceNote = "pc_item_004_amulet";
        ...
        public static ItemDefinition ParseRow(string[] cols, int itemIdHint = 0)
        {
            ...
            if (PcItemCommon.ContainsReplacementChar(nameRaw))
                item.warnings.Add($"Amulet row id={itemId} contains Unicode replacement char");
            return item;
        }
    }
}
