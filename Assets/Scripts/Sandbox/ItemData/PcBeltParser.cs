// -----------------------------------------------------------------------------
// VLTK Mobile — PC belt.txt data-driven port
// Source: server/settings/item/004/belt.txt (GB2312, 46 tab-separated columns)
// Purpose: keep the PC belt (đai lưng) catalog visible to mobile runtime by
    ...
    public static class PcBeltParser
    {
        ...
        public static ItemDefinition ParseRow(string[] cols, int itemIdHint = 0)
        {
            ...
            if (PcItemCommon.ContainsReplacementChar(nameRaw))
                item.warnings.Add($"Belt row id={itemId} contains Unicode replacement char");
            return item;
        }
    }
}
