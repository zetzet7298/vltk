// -----------------------------------------------------------------------------
// VLTK Mobile — PC pendant.txt data-driven port
// Source: server/settings/item/004/pendant.txt (GB2312, 46 tab-separated columns)
// Purpose: keep the PC pendant (liên / dây chuyền) catalog visible to mobile
...
{
    public static class PcPendantParser
    {
        public const int MinColumns = 46;
        public const int StatCount = 7;
        public const int ReqCount = 6;
        public const string EvidenceNote = "pc_item_004_pendant";
        ...
        public static ItemDefinition ParseRow(string[] cols, int itemIdHint = 0)
        {
            ...
            if (PcItemCommon.ContainsReplacementChar(nameRaw))
                item.warnings.Add($"Pendant row id={itemId} contains Unicode replacement char");
            return item;
        }
    }
}
