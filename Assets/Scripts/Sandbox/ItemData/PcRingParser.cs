// -----------------------------------------------------------------------------
// VLTK Mobile — PC ring.txt data-driven port
// Source: server/settings/item/004/ring.txt (GB2312, 46 tab-separated columns)
// Purpose: keep the PC ring (nhẫn) catalog visible to mobile runtime by
    ...
    public static class PcRingParser
    {
        ...
        public static ItemDefinition ParseRow(string[] cols, int itemIdHint = 0)
        {
            ...
            if (PcItemCommon.ContainsReplacementChar(nameRaw))
                item.warnings.Add($"Ring row id={itemId} contains Unicode replacement char");
            return item;
        }
    }
}
