// -----------------------------------------------------------------------------
// VLTK Mobile — PC boot.txt data-driven port
// Source: server/settings/item/004/boot.txt (GB2312, 46 tab-separated columns)
// Purpose: keep the PC boot (giày) catalog visible to mobile runtime by id/name/
// icon and stat deltas (base + refine).
// -----------------------------------------------------------------------------
    ...
    public static class PcBootParser
    {
        ...
        public static ItemDefinition ParseRow(string[] cols, int itemIdHint = 0)
        {
            ...
            if (PcItemCommon.ContainsReplacementChar(nameRaw))
                item.warnings.Add($"Boot row id={itemId} contains Unicode replacement char");
            return item;
        }
    }
}
