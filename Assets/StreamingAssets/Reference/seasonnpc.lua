...
if (tl_gettaskcourse() == 2) then
	...
	if (tl_gettaskcourse() == 3) then
		...
		Say("<color=green>D· TÈu:<color><enter>VŞ "..GetPlayerSex().." nµy ®· hoµn thµnh tÊt c¶ ®­îc <color=yellow>"..myCountTimes.."<color>, nhiÖm vô nµy<enter>ngµy h«m nay cã thÓ thùc hiÖn l¹i<color=yellow> "..(So_Lan_Da_Tau_Trong_Ngay - GetTask(ID_TASKLINK_LIMITNUM)).."<color>, ng­¬i cã muèn <enter>lµm n÷a kh«ng?",2,"§­¬ng nhiªn, mau cho ta biÕt nhiÖm vô tiÕp theo lµ g×/Task_TaskProcess","§Ó ta nghØ ng¬i mét l¸t ®·! Ta bËn råi/Task_Wait");
