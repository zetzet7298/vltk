...
--	player_Faction = GetFaction()
	if (GetSeries() == 2) and (GetFaction() == "emei") then
		if (UTask_em == 10*256) and (GetLevel() >= 10) then		--10¼¶ÈÎÎñÆô¶¯
			Talk(5,"L10_talk2", "PhËt Tæ! §Ö tö cã téi, xin h·y trõng ph¹t ®Ö tö, ®õng gi¸ng téi cho huynh Êy....", "S­ tû, xem tû ®uém nÐt u sÇu, cã chuyÖn g× kh«ng?", "Nãi ra dµi l¾m -- Tr­íc khi ta xuÊt gia ®Çu PhËt, ®· tõng cã mét gia ®×nh h¹nh phóc ngËp trµn. Nµo ngê cuéc chiÕn Tèng Kim næ ra, t­íng c«ng ta bÞ l«i ra chiÕn tr­êng lµm Tr¸ng §inh. Chµng ra ®i nhiÒu n¨m nh­ng kh«ng cã tin tøc g×!", "VÒ sau cã tin chµng ®· tö trËn n¬i chiÕn tr­êng. Ta v« cïng ®au khæ, xuèng tãc lµm ni c«, ®Þnh sèng cuéc sèng ®¹m b¹c trong qu·ng ®êi tµn. Nµo ngê, t­íng c«ng ta vÉn cßn sèng!", "Sau khi chµng trë vÒ, ®au khæ khÈn thiÕt mong ta hoµn tôc. Nh­ng, ta ®· ph¸t nguyÖn n­¬ng nhê cöa PhËt, sao cã thÓ v× chuyÖn luyÕn ¸i hång trÇn mµ quay l­ng l¹i víi PhËt Tæ? Ta thËt cã téi! ")
		...
		elseif ((UTask_em == 10*256+60) and (HaveItem(20) == 1)) then					--µÃµ½ÐÞ¸´ºÃµÄÍêÕûÍ­¾µ
			Talk(2, "", "S­ tû, nh×n xem! G­¬ng ®ång ®· söa xong! G­¬ng vì l¹i lµnh, nÕu PhËt Tæ biÕt tÊm ch©n t×nh cña phu phô tû, nhÊt ®Þnh sÏ kh«ng tr¸ch tû ®©u!", "§a t¹ s­ muéi! H·y gióp ta mang tÊm g­¬ng ®ång trao cho t­íng c«ng. Nh¾n víi chµng nÕu b»ng lßng ®îi ta 3 n¨m. 3 n¨m sau ta nhÊt ®Þnh trë vÒ bªn chµng. Trong 3 n¨m ta ph¶i chuyªn t©m tu hµnh ®Ó cÇu xin PhËt Tæ tha thø.")
			SetTask(1,10*256+70)
		...
		elseif ((UTask_em == 10*256+70) and (HaveItem(20) == 0)) then		-- ÈÎÎñÖÐ£¬Í­¾µ¡°ÓÖ¡±¶ªÁË
			AddEventItem(20)
	...
	elseif (UTask_em >= 70*256) then								--ÒÑ¾­³öÊ¦
		Talk(1,"","Muéi ph¶i h¹ s¬n sao? H·y chó ý b¶o träng.")
	else
...
function L10_talk2()
	Talk(3,"L10_get","S­ tû! §õng tù tr¸ch m×nh! Cã tr¸ch h·y tr¸ch chuyÖn ®êi ngang tr¸i, t¹o hãa trªu ng­êi.", "NÕu ta kh«ng hoµn tôc, chµng sÏ kh«ng chÞu rêi khái Nga Mi. Chµng nãi ngµy ngµy sÏ ®øng d­íi nói ®îi ta, mong ta håi t©m chuyÓn ý. Ta ph¶i lµm sao ®©y? Lßng ta rèi nh­ t¬ vß!", "S­ tû, t©m ý s­ tû rèt cuéc lµ thÕ nµo? Tû cßn yªu t­íng c«ng kh«ng?")
end;

function L10_get()
	Say("NÕu t×nh duyªn kiÕp nµy ®· ®Þnh s½n nh­ thÕ, ta lµm sao d¸m c­ìng l¹i? N¨m x­a khi chµng tßng qu©n, ®· tõng chia m¶nh g­¬ng lµm 2 m¶nh ®Ó lµm tÝn vËt cho ngµy sau gÆp l¹i, lÏ nµo g­¬ng vì l¹i lµnh sao?", 2, "§Ó ta thö xem. /L10_get_yes", "Kh«ng cã c¸ch/L10_get_no")
end;
...
--	AddMagic(77)
--	AddMagic(79)
	add_em(20)			-- µ÷ÓÃskills_table.luaÖÐµÄº¯Êý£¬²ÎÊýÎªÑ§µ½¶àÉÙ¼¶¼¼ÄÜ¡£
