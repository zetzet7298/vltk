if (not __FESTIVAL_HEAD__) then
	...
	FE_SMALLMAPCOUNT = 19;	--Ò»¹²ÓÐ12ÕÅÐ¡µØÍ¼
	FE_MOUSECOUNT_MIN = 10;	--Ò»¸öÐ¡µØÍ¼Ò»´Î×îÉÙË¢µØÊóÊýÄ¿
	FE_MOUSECOUNT_MAX = 10;	--Ò»¸öÐ¡µØÍ¼Ò»´Î×î¶àË¢µØÊóÊýÄ¿
	FE_MAXPLAYERCOUNT = 10;	--Ã¿¸öÓÎÏ·×î¶à10¸öÈË£»
	FE_NIANSHOU_XUANYUN = 10;   --ÄêÊÞÃ¿±»¹¥»÷10´Î¾Í±»Ñ£ÔÎ
	...
	TK_FE_PLAYTIMES = 1127; --¼ÇÂ¼Ò»ÌìÄÚÍæµÄ´ÎÊý
	...
	TK_FE_EXP_BEISHU = 1131; -- »ñµÃ¾­ÑéµÄ±¶Êý
	TK_FE_EXP_TIME = 1132;  -- »ñµÃË«±¶µÄÊ±¼ä
	
	TK_FE_ATTACKED_TIMES = 1133;    --¼ÇÂ¼ÄêÊÞ±»¹¥»÷µÄ´ÎÊý
	...
	--MS²¿·Ö£»
	MS_FE_STATE = 1;
	MS_FE_TIMERSTATE = 2; --ÓÃÀ´¼ÇÂ¼ÓÃÓÚ½»ÌæµÄ½øÐÐË¢¹ÖºÍÇå³ý¹Ö£º0£­Çå³ý¹Ö£»1£­Ë¢³ö¹Ö
	MS_FE_NEWSVALUE = 3; --
	...
	function festival_clearfairy(szNPCName)
		...
		local OldWorld = SubWorld;
		SubWorld = SubWorldID2Idx(FE_MAPID[2]);
		ClearMapNpcWithName(FE_MAPID[2], szNPCName);     --½«ËùÓÐµÄÄêÊÞÇå³ý
		SubWorld = OldWorld;
	end;
	...
	function festival_setplayertoprepare()	--Íæ¼Ò½øÈë×¼±¸³¡Ê±ºò½øÐÐ×´Ì¬ÉèÖÃ
	...
	function festival_setplayerleaverstate()	--Íæ¼ÒÀë¿ªÓÎÏ·Ê±ºò½øÐÐ×´Ì¬ÉèÖÃ
	...
	function festival_setplayertogame()	--Íæ¼Ò½øÈëÓÎÏ·µÄÊ±ºò½øÐÐ×´Ì¬ÉèÖÃ
		ForbitSkill(FESTIVAL_OK);
		SetMoveSpeed(FESTIVAL_ENTERSPEED);
	...
	function festival_check_pl(szPlayName)
		...
		if (nPlIdx > 0) then
			local nPlMsIdx = PIdx2MSDIdx(FE_MISSIONID, nPlIdx)	--Íæ¼ÒÊÇ·ñÔÚÖ¸¶¨MissionÖÐ
			if (nPlMsIdx > 0) then
				return nPlIdx
			end
		end
