...
function SecondHandStore_Agent:SellGoodsToPlayer(szName, tbGoods)
	...
	if not tbGoods or tbGoods.nPrice <= 0 then --ÀíÂÛÉÏtbGoods¾ø¶ÔºÏ·¨
		return 
	end
	local nPlayerIndex = SearchPlayer(szName)
	...
	if nPlayerIndex > 0 then
		if lib:DoFunByPlayer(nPlayerIndex, CountFreeRoomByWH, tbGoods.nWidth, tbGoods.nHeight, 1) >= 1 then
			if lib:DoFunByPlayer(nPlayerIndex,Pay, tbGoods.nPrice) == 1 then
				...
				self:Msg2Player(szName, "Ng©n l­îng kh«ng ®ñ! ")
			end	
		else
			self:Msg2Player(szName, "Hµnh trang kh«ng ®ñ chç trèng! ")
		end
...
function SecondHandStore_Agent:ReceiveGoods(nParam, ResultHandle)
	...
	if bSuccess then --³É¹¦»ñÈ¡µÄ»° varDataÎªÒ»¸öÉÌÆ·£¬·ñÔòÊÇ´íÎóÐÅÏ¢
		 --³É¹¦Âô¸øÍæ¼Ò£¬ÔòÍ¨ÖªrelayÉ¾³ý¸ÃµÀ¾ß£¬·ñÔòÍ¨Öªrelay½âËø¸ÃµÀ¾ß
