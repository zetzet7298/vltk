...
function ShoutuList(apps)
	local str = "Xem t×nh h×nh cña ng­êi ch¬i, chän 1 ng­êi thÝch hîp lµm ®å ®Ö cña b¹n:"
	local btn = {}
	-- local apps = FindApprentice()
	if(not apps) then
		Msg2Player("Kh«ng cã ®å ®Ö thÝch hîp.")
		return
	end
...
--µÃµ½¿ÉÒÔ×öÊ¦¸¸µÄÈËÔ±ÁÐ±í
function BaishiList(masters)
	print("Call BaishiList()")
	local str = "Tra xem t×nh h×nh cña ng­êi ch¬i, chän 1 ng­êi thÝch hîp lµm s­ phô cña b¹n:"
	local btn = {}
...
--ÁË½âÄÜ¹»³ÉÎªÍ½µÜµÄÈËÑ¡µÄÐÅÏ¢
function XuanTudi(playerName)
	local pid = SearchPlayer(playerName)
	if(FALSE(pid)) then
		Msg2Player(playerName.."HiÖn kh«ng biÕt tung tÝch.")
		return
	end
...
function Baishi(master) --Í½µÜ´¥·¢
	...
	if(FALSE(pid)) then
		Msg2Player("Xin lçi! "..master.."HiÖn kh«ng biÕt tung tÝch.")
		return
	end
	...
	if(GetTaskTemp(TTKID_APPRENTICE_FLAG)==GetTaskTemp(TTKID_RECRUIT_FLAG)) then
		...
		Msg2Player(master.." ®· ®ång ý thu nhËn b¹n lµm ®å ®Ö, ®îi hÖ thèng x©y dùng quan hÖ s­ ®å cña c¸c b¹n.")
	...
	Msg2Player("B¹n ®Þnh b¸i "..master.." lµm s­ phô, ®ang ®îi tr¶ lêi...")
	sendMessage(pid,GetName().." ®Þnh b¸i b¹n lµm s­ phô, xin sö dông chøc n¨ng <color=yellow>thu nhËn ®Ö tö<color> trong <color=green>S­ ®å thiÕp<color> ®Ó tr¶ lêi.")
	return
end
...
function Shoutu(apprentice) --Ê¦¸¸´¥·¢
	...
	if(FALSE(pid)) then
		Msg2Player("Xin lçi! "..apprentice.."HiÖn kh«ng biÕt tung tÝch.")
		return
	end
	...
	if(appinfo.tasktmp[TTKID_APPRENTICE_FLAG]==appinfo.tasktmp[TTKID_RECRUIT_FLAG]) then
		...
		Msg2Player("B¹n ®· ®ång ý thu nhËn "..apprentice.." lµm ®å ®Ö, ®îi hÖ thèng x©y dùng quan hÖ s­ ®å cña c¸c b¹n.")
	...
	Msg2Player("B¹n ®Þnh thu nhËn"..apprentice.." lµm ®å ®Ö, ®ang ®îi tr¶ lêi...")
	sendMessage(pid,GetName().."®Þnh thu nhËn b¹n lµm ®å ®Ö. H·y sö dông chøc n¨ng <color=green>B¸i s­<color> trong <color=green>S­ ®å thiÕp"..GetName().."<color> tr¶ lêi.")
	return
end
...
function TuConform()
	...
	if (masterName == nil) then
		...
		if(TRUE(lid)) then -- Ê¦¸¸²»´æÔÚ£¬µ«Õ½¶Ó´æÔÚ
			Say("S­ phô cña b¹n ®· mÊt tÝch, b¹n quyÕt ®Þnh hñy bá quan hÖ s­ ®å víi s­ phô?",getn(btn),btn)
		else
...
function ChaLilian(appName) --Ê¦¸¸´¥·¢
	...
	if(FALSE(appIdx)) then
		Msg2Player(appName.."Kh«ng biÕt tung tÝch.")
		return
	end
...
function FuLilian(masterName) --Í½µÜµ÷ÓÃ
	...
	if(FALSE(masterIdx)) then
		Msg2Player(masterName.."Kh«ng biÕt tung tÝch.")
		return
	end
...
function SendBaiShiBidding()
	...
	local str = "ng­êi ch¬i míi ["..GetName().."] t­ chÊt h¬n ng­êi! XuÊt s­ nhí ®õng lµm hæ danh s­ phô nhÐ!"
	if (myMapName ~= 0) then
		x = floor(x / 8);
		y = floor(y / 16);
		AddLocalNews(str.."ng­êi cã ý h·y ®Õn ["..myMapName.."]("..x..", "..y..")");
	else
...
function ApprenticeDialog()
	...
	-- VN_MODIFY_20060427
	-- ¹Ø±Õ»Æ½ðÊ¦Í½¿¨(ËµÃ÷)
	-- tinsert(btn,"ÁË½â»Æ½ðÊ¦Í½¿¨(ÌØÐÔËµÃ÷)/AboutShituGoldCard")
	tinsert(btn,"§ãng/Quit")
...
function ChushiList()
	...
	if(not apps) then
		Msg2Player("HiÖn kh«ng cã ®Ö tö thÝch hîp cã thÓ xuÊt s­.")
		return
	end
	local btn={}
	for i=1,getn(apps) do
...
function ShiChushi(appName)
	local appIdx = SearchPlayer(appName)
	if(FALSE(appIdx)) then
		Msg2Player(appName.."Kh«ng biÕt tung tÝch, xem ra t¹m thêi kh«ng thÓ ®Ó ®å ®Ö xuÊt s­.")
		return
	end
	...
	Msg2Player("B¹n c¶m thÊy"..appName.."Th«ng minh lanh lîi, chíp m¾t ®· kh«ng cßn g× d¹y ®Ö tö n÷a, quyÕt ®Þnh ®Ó ®Ö tö xuÊt s­,"..appName.."trong lóc suy nghÜ...")
	sendMessage(appIdx,GetName().."c¶m thÊy b¹n ®· ­u tó h¬n thÇy chíp m¾t ®· kh«ng cßn g× d¹y b¹n n÷a, quyÕt ®Þnh ®Ó b¹n xuÊt s­, ®ang ®îi quyÕt ®Þnh cña b¹n...")
	return
end
...
function TudiInfo(appName)  --Ê¦¸¸²éÍ½µÜÐÅÏ¢
	local pid = SearchPlayer(appName)
	if(FALSE(pid)) then
		Msg2Player(appName.."Kh«ng biÕt tung tÝch, kh«ng thÓ nhËn ®­îc tin tøc cña")
		return
	end
	local sex = callPlayerFunction(pid,GetSex)
	local str
	if(FALSE(sex)) then --ÄÐ
		str = "<link=image[0,0]:\\spr\\Ui3\\Íæ¼ÒÐ¤Ïñ\\Ð¤Ïñ\\001.spr>"
	else
		str = "<link=image[0,0]:\\spr\\Ui3\\Íæ¼ÒÐ¤Ïñ\\Ð¤Ïñ\\003.spr>"
	end
	...
	if(masterCount<=1) then
		...
		if(joinLvl<=25) then
			str = str.."B¹n ch¬i lµ ®Ö tö <color=yellow>§Ých truyÒn<color> cña b¹n."
		else
	...
	if(lilianCount<1) then
		if(TRUE(newllc)) then
			str = str..appName.."®· hoµn thµnh nh­ sau<color=yellow>"..newllc.."<color> NhiÖm vô thÝ luyÖn giang hå, ®ang ®îi b¹n kiÓm tra:<enter>"
		...
		if(FALSE(masterllCount)) then --»¹Ã»¸´ÃüÈÎºÎÀúÁ·
			if(TRUE(newllc)) then
				str = str.."HiÖn t¹i"..appName.."®· hoµn thµnh nh­ sau<color=yellow>"..newllc.."<color> NhiÖm vô thÝ luyÖn giang hå, ®ang ®îi b¹n kiÓm tra:<enter>"
				local newll = GetNewLLInfo(GetPlayerNewLilian(pid))
				for i=1,getn(newll) do
					str = str.."  "..newll[i].."<enter>"
				end
			end
			str = str.."Tr­íc kia, "..appName.."®· hoµn thµnh."..lilianCount.."nhiÖm vô thÝ luyÖn giang hå, "
			str = str.."nh­ng ë tªn tuæi cña b¹n, ch­a hoµn thµnh nhiÖm vô thÝ luyÖn giang hå nµo.<enter>"
		else
			if(TRUE(newllc)) then
				str = str..appName.."®· hoµn thµnh nh­ sau<color=yellow>"..newllc.."<color> NhiÖm vô thÝ luyÖn giang hå, ®ang ®îi b¹n kiÓm tra:<enter>"
				local newll = GetNewLLInfo(GetPlayerNewLilian(pid))
			...
			str = str.."Tr­íc kia, "..appName.."cßn hoµn thµnh"..lilianCount.."nhiÖm vô thÝ luyÖn giang hå, "
			str = str.."Trong sè nhiÖm vô hoµn thµnh mµ b¹n cã nh­<color=green>"..masterllCount.."<color> nhiÖm vô thÝ luyÖn giang hå:<enter>"
	...
	if(unfinishedLilian) then
		...
		str = str.."§Ö tö ®· hoµn thµnh tÊt c¶ nhiÖm vô thÝ luyÖn giang hå!"
	end
	local btn = {
...
function ChaSelfInfo() --Í½µÜ²é×Ô¼ºµÄÐÅÏ¢
	local str
	if(FALSE(GetSex())) then --ÄÐ
		str = "<link=image[0,0]:\\spr\\Ui3\\Íæ¼ÒÐ¤Ïñ\\Ð¤Ïñ\\001.spr>"
	else
		str = "<link=image[0,0]:\\spr\\Ui3\\Íæ¼ÒÐ¤Ïñ\\Ð¤Ïñ\\003.spr>"
	end
	...
	if(lilianCount<1) then
		if(TRUE(newllc)) then
			str = str.."b¹n ®· hoµn thµnh <color=yellow>"..newllc.."<color> nhiÖm vô thÝ luyÖn giang hå, ®ang chêphôc mÖnh víi s­ phô:<enter>"
		...
		if(FALSE(masterllCount)) then --Î´ÔÚÎÒÃûÏÂÍê³ÉÁËÈÎÎñ  
			if(TRUE(newllc)) then
				str = str.."HiÖn b¹n ®· hoµn thµnh <color=yellow>"..newllc.."<color> nhiÖm vô thÝ luyÖn giang hå, ®ang chêphôc mÖnh víi s­ phô:<enter>"
				local newll = GetNewLLInfo(GetPlayerNewLilian(PlayerIndex))
				for i=1,getn(newll) do
					str = str.."  "..newll[i].."<enter>"
				end
			end
			str = str.."Tr­íc kia, b¹n ®· hoµn thµnh."..lilianCount.."nhiÖm vô thÝ luyÖn giang hå, "
			str = str.."nh­ng d­íi tªn tuæi cña s­ phô, b¹n ch­a hoµn thµnh nhiÖm vô thÝ luyÖn giang hå nµo.<enter>"
		else
			if(TRUE(newllc)) then
				str = str.."b¹n ®· hoµn thµnh <color=yellow>"..newllc.."<color> nhiÖm vô thÝ luyÖn giang hå, ®ang chêphôc mÖnh víi s­ phô:<enter>"
				local newll = GetNewLLInfo(GetPlayerNewLilian(PlayerIndex))
			...
			str = str.."Tr­íc kia, b¹n ®· hoµn thµnh."..lilianCount.."nhiÖm vô thÝ luyÖn giang hå, "
			str = str.."trong sè nhiÖm vô hoµn thµnh nh­ sau<color=green>"..masterllCount.."<color> nhiÖm vô thÝ luyÖn giang hå:<enter>"
	...
	if(unfinishedLilian) then
		...
		str = str.."B¹n ®· hoµn thµnh tÊt c¶ nhiÖm vô thÝ luyÖn giang hå!"
	end
	local btn = {
...
function TdChaShifu()
	...
	local masterIdx = SearchPlayer(masterName)
	if(FALSE(masterIdx)) then
		Say("S­ phô cña b¹n"..safeshow(masterName).." hiÖn ®· mÊt tÝch, b¹n kh«ng thÓ t×m hiÓu t×nh h×nh cña s­ phô.",getn(btn),btn)
		return
	end
...
function DefaultDialog()
	...
	local szOp = {
		...
		"T×m hiÓu S­ ®å thiÕp hoµng kim (thuyÕt minh ®Æc tÝnh) /AboutShituGoldCard",
		"§ãng/Quit"
	};
