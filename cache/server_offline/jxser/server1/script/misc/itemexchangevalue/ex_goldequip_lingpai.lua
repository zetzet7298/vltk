...
-- ÎÄ¼şÃû¡¡£ºex_goldequip_lingpai.lua
-- ´´½¨Õß¡¡£º×Ó·Çô~
...
IncludeLib("ITEM")
Include("\\script\\misc\\itemexchangevalue\\ex_goldequip_head.lua")	-- ¶Ò»»ĞÂ×°±¸ÁîÅÆ£¨×ÏòşµÈ£©
...
function exchange_lingpai2goldequip()
	local aryDescribe =
	{
		...
		format("Bé trang bŞ Thanh C©u/#take_xp_equip_1(1,%d)", 905),
		...
		format("Bé trang bŞ Minh Ph­îng/#take_xp_equip_1(9, %d)", 2745),
		format("Bé trang bŞ §»ng Long/#take_xp_equip_1(10, %d)", 2775),
...
function take_xp_equip_1(n_g, n_d)
	...
	CreateTaskSay({"<dec><npc>§¹i hiÖp muèn ®æi trang bŞ cña hÖ ph¸i nµo bªn d­íi:", 
...
function take_xp_equip_11(n_g, n_d)
	CreateTaskSay({"<dec><npc>§¹i hiÖp muèn ®æi trang bŞ cña hÖ ph¸i nµo bªn d­íi:", 
						...
						format("Vâ §ang khİ/#take_xp_equip_2(%d,%d, 20)", n_g, n_d),
						...
						format("Hoa S¬n Khİ/#take_xp_equip_2(%d,%d, 24)", n_g, n_d),
...
function take_xp_equip_2(n_g, n_d, n_p)
	...
	local aryDescribe = {
		"<dec><npc>§¹i hiÖp muèn ®æi trang bŞ cña hÖ ph¸i nµo bªn d­íi:",
	};
...
function take_xp_equip_3(n_g, n_d, n_p, n_x)
	if (CountFreeRoomByWH(2,4,1) < 1) then
		Talk(1, "", "Hµnh trang kh«ng ®ñ « trèng! Xin h·y s¾p xÕp råi quay l¹i nhĞ.");
		return
	end
...
TIME_2000_1_1 = 946656000;		-- 2000Äê1ÔÂ1ÈÕ Ïà¶ÔÓÚ 1970Äê1ÔÂ1ÈÕµÄÃëÊı
