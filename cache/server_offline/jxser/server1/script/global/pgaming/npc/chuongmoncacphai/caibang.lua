...
function main()
	...
	tbDailog:AddOptEntry("N©ng cÊp Trang BÞ TuyÖt §Ønh C¸i Bang", tbHKUpgrade.CaiBang, {tbHKUpgrade})
	tbDailog:AddOptEntry("N©ng cÊp Ên TuyÖt §Ønh C¸i Bang", tbAnUpgrade.CaiBang, {tbAnUpgrade})
...
function oldentence()
	...
	if (UTask_gb == 70*256) and (GetFaction() == "gaibang") then
		SetFaction("")
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kÞp thêi håi phuc!",0)
		return
	elseif (UTask_gb == 70*256) and (GetTask(4) >= 5*256) and (GetTask(4) < 10*256) then
		SetTask(4,0)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kÞp thêi håi phuc!",0)
		return
	elseif (UTask_gb == 80*256) and (GetCamp() == 4) then
		SetTask(8,70*256)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kÞp thêi håi phuc!",0)
		return
	elseif (GetTask(4) == 70*256) and (GetTask(8) == 70*256) then
		SetTask(4,75*256)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kÞp thêi håi phuc!",0)
		return
...
function common_talk()
	...
	elseif (GetSeries() == 3) and (GetFaction() == "gaibang") then
		if (UTask_gb == 50*256+60) then
			Talk(1,"L50_prise","ChuyÖn nµy lµ nh­ vËy ®ã, ®Ö tö ®· ®em tÊm Trung Nguyªn ®Þa ®å giao cho Tr­¬ng §¹i Nh©n, nhê «ng ta tr×nh lªn Hoµng Th­îng.")
		elseif ((UTask_gb == 50*256+10) or (UTask_gb == 50*256+20)) and (HaveItem(199) == 0) then
			Talk(2,"","Sao ng­¬i cÈu th¶ thÕ nµy! Ta dÆn ®i dÆn l¹i mÊy lÇn råi ph¶i cÈn thËn gi÷ bøc ®Þa ®å chø, sao cã thÓ ®¸nh r¬i ®­îc!","ë ®©y cßn mét b¶n, lÇn nµy ph¶i cÈn thËn h¬n ®ã, ®õng ®Ó x¶y ra chuyÖn n÷a!")
			AddEventItem(199)
		elseif ((UTask_gb == 50*256) and (GetLevel() >= 50)) then
			Say("Trong cÈm hép nµy lµ mét bøc tranh miªu t¶ Trung Nguyªn ®Þa ®å, cÇn ng­êi ®em ®Õn L©m An d©ng cho triÒu ®×nh. Ng­¬i cã ®ång ý ®i kh«ng?",2,"§ång ý!/L50_get_yes","§Ö tö cßn yÕu kÐm qu¸! E kh«ng ®­¬ng nçi viÖc nµy!/no")
		elseif (UTask_gb == 10*256+40) and (HaveItem(76) == 1) and (HaveItem(77) == 1) and (HaveItem(78) == 1) and (HaveItem(79) == 1) and (HaveItem(80) == 1) then
			L10_prise()
		elseif (UTask_gb == 10*256) and (GetLevel() >= 10) then
		...
		elseif (UTask_gb == 80*256) then
			Say("KhiÕu hãa tö ta ®©y chÝ t¹i bèn ph­¬ng ng­¬i ®õng suèt ngµy ngåi kh«ng ra ®Êy!",3,"D¹! §Ö tö muèn ra giang hå rÌn luyÖn thªm!/goff_yes","§Ö tö tù thÊy m×nh vâ nghÖ cßn kÐm, ch­a thÓ xuÊt s­!/no")
	...
	elseif (GetSeries() == 3) and (GetCamp() == 4) and (GetLevel() >= 60) and (UTask_gb == 70*256) and ((GetTask(4) < 5*256) or (GetTask(4) == 75*256)) then		-- ÖØ·µÊ¦ÃÅÈÎÎñ
		Talk(3,"return_sele","KhiÕu hãa tö bªn ngoµi chÞu nhiÒu lêi ®µm tiÕu qu¸!","§­¬ng nhiªn, cho c¬m ng­¬i ¨n ph¶i cã chót g× ®ã chø, chuyÖn ®­¬ng nhiªn th«i, haha!","Th­êng kh«ng cho c¬m ¨n cßn b¾t chóng t«i ra ®Ó trót giËn n÷a ®Êy!")
	elseif (GetCamp() == 4) and ((UTask_gb == 70*256+10) or (UTask_gb == 70*256+20)) then
		Say("5 v¹n l­îng ®· chuÈn bÞ xong ch­a vËy?",2,"§· chuÈn bÞ xong/return_complete","VÉn ch­a chuÈn bÞ xong/return_uncompleted")
	else
...
function return_yes()
	Talk(2,"","ChØ t¹i bæn bang chi c¸c kho¶n lín, th­êng kh«ng ®ñ xoay së, nÕu ng­¬i muèn trïng ph¶i bæn ph¸i th× ph¶i quyªn gãp 50000 l­îng.","§­îc! §Ö tö chuÈn bÞ tiÒn ngay ®©y.")
	SetTask(8,70*256+20)
...
function return_no()
	Talk(1,"","S­ phô, con c¶m thÊy sèng trªn ®êi ph¶i gÇy dùng sù nghiÖp, kh«ng thÓ ¨n b¸m ng­êi kh¸c qua ngµy ®­îc. V× vËy cßn tÝnh kh«ng quay l¹i ®©u.")
end
...
function L10_get_yes()
	SetTask(8,10*256+10)
	AddNote("TiÕp nhËn nhÞªm vô cÊp 10, ®i mua 5 lo¹i r­îu HuÖ TuyÒn Töu, Kim L¨ng Xu©n, B¸ch Hoa Nh­ìng, Kinh Hoa Lé, Song C©u §¹i Khóc vÒ cho Bang chñ.")
	Msg2Player("TiÕp nhËn nhÞªm vô cÊp 10, ®i mua 5 lo¹i r­îu HuÖ TuyÒn Töu, Kim L¨ng Xu©n, B¸ch Hoa Nh­ìng, Kinh Hoa Lé, Song C©u §¹i Khóc vÒ cho Bang chñ.")
end
...
function L50_get_yes()
	Talk(1,"","TÊm Trung Nguyªn ®Þa ®å nµy quan hÖ träng ®¹i, cÈn thËn ®õng ®Ó lät vµo tay ng­êi Kim ®Êy!")
	SetTask(8,50*256+10)
	AddEventItem(199)
	AddNote("TiÕp nhËn nhiÖm vô cÊp 50, hiÕn tÆng bøc §Þa ®å Trung nguyªn cho triÒu ®×nh.")
	Msg2Player("TiÕp nhËn nhiÖm vô cÊp 50, hiÕn tÆng bøc §Þa ®å Trung nguyªn cho triÒu ®×nh.")
end
