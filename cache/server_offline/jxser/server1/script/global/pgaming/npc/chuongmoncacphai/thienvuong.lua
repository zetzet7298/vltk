...
function main()
	...
	tbDailog:AddOptEntry("N©ng cÊp Trang BÞ TuyÖt §Ønh Thiªn V­¬ng", tbHKUpgrade.ThienVuong, {tbHKUpgrade})
	tbDailog:AddOptEntry("N©ng cÊp Ên TuyÖt §Ønh Thiªn V­¬ng", tbAnUpgrade.ThienVuong, {tbAnUpgrade})
...
function oldentence()
	...
	if (UTask_tw == 70*256) and (GetFaction() == "tianwang") then
		SetFaction("")
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kÞp thêi håi phuc!",0)
		return
	elseif (UTask_tw == 70*256) and (GetTask(7) >= 5*256) and (GetTask(7) < 10*256) then
		SetTask(7,0)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kÞp thêi håi phuc!",0)
		return
	elseif (UTask_tw == 80*256) and (GetCamp() == 4) then
		SetTask(3,70*256)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kÞp thêi håi phuc!",0)
		return
...
function common_talk()
	...
	if (GetTask(39) == 10) and (GetBit(GetTask(40),3) == 0) then
		Talk(1,"","TriÒu ®×nh nhµ Tèng ngu xuÈn, kh«ng ®éi trêi chung víi D­¬ng gia ta. N¨m x­a Së V­¬ng cïng cha ta thÒ quyÕt diÖt Tèng. D­¬ng Anh tuy bÊt tµi, nh­ng còng quyÕt thùc hiÖn chÝ lín!")
		Uworld40 = SetBit(GetTask(40),3,1)
		SetTask(40,Uworld40)
	elseif (GetSeries() == 0) and (GetFaction() == "tianwang") then
		if (UTask_tw == 60*256+40) and (HaveItem(96) == 1) then
		...
		elseif (UTask_tw > 60*256) and (UTask_tw < 70*256) then
			Talk(1,"","<color=Red>'Thiªn V­¬ng Di th­'<color> ®­îc giÊu trong mét <color=Red>B¶o r­¬ng <color>, B¶o r­¬ng ®­îc giÊu ë <color=Red>phÝa b¾c Thanh Loa ®¶o<color>, tr­íc tiªn cÇn ph¶i t×m ®­îc <color=Red>Ch×a khãa ®Ó më B¶o r­¬ng<color>.")
		else
			Talk(1,"","Kim quèc cã ý muèn ng«ng cuång lËt ®æ vâ l©m Trung Nguyªn ta, nÕu nh­ huynh ®Ö chóng ta kh«ng ®ång t©m, lµm sao cã thÓ chèng ®­îc qu©n thï?")
		end
	elseif (GetSeries() == 0) and (GetCamp() == 4) and (GetLevel() >= 60) and (UTask_tw == 70*256) and (GetTask(7) < 5*256) then
	...
	elseif (GetCamp() == 4) and ((UTask_tw == 70*256+10) or (UTask_tw == 70*256+20)) then
		Say("Ng­¬i ®· chuÈn bÞ ®ñ 5 v¹n l­îng ch­a?",2,"§· chuÈn bÞ xong/return_complete","VÉn ch­a chuÈn bÞ xong/no")
	elseif (UTask_tw >= 70*256) and (GetFaction() ~= "tianwang") then
		Talk(1,"","Sau khi rêi khái Thiªn V­¬ng Bang, mäi ng­êi vÉn nh­ anh em ruét thÞt!")
	else
		Talk(1,"","Thiªn V­¬ng Bang ®øng gi÷a mèi thï Tèng Kim vÉn ®øng v÷ng, thËm chÝ vÉn ph¸t triÓn m¹nh mÏ, ®Òu do huynh ®Ö trong Bang ®oµn kÕt mét lßng")
	end
end
...
function check_skill()
	...
	if (x > 0) then
		Say("Vi s­ lÇn nµy bÕ quan suy nghÜ mÊy ngµy, s¸ng chÕ ra ®­îc chiªu thøc míi, b©y giê truyÒn l¹i cho ng­¬i. Häc xong nhí ph¶i tÞnh d­ìng mét thêi gian ®Ó khái lµm tæn th­¬ng kinh m¹ch.",1,"§a t¹ s­ phô /KickOutSelf")
		return 0
	else
		return 1
	end
...
function defection_yes()
	n = 0
	i=29 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=23 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=24 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=26 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=30 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=31 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=32 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=33 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=34 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=35 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=36 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=37 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=38 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=40 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=41 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
...
function defection_no()
	SetTask(7,1*256)
	AddNote("B¹n tõ bá ý ®Þnh gia nhËp ThiÕu L©m ph¸i. ")
	Msg2Player("B¹n tõ bá ý ®Þnh gia nhËp ThiÕu L©m ph¸i. ")
end
...
function return_yes()
	Talk(2,"","RÊt tèt! Tr­íc m¾t qu©n ta ®ang giao chiÕn víi qu©n Tèng, qu©n phÝ kh«ng ®ñ, ng­¬i nép 50000 l­îng th× cã thÓ trë l¹i bæn bang häc Kim Chung Tr¸o","V©ng! Xin ®Ó ®Ö tö chuÈn bÞ mét chót. ")
	SetTask(3,70*256+20)
	AddNote("Hæ trî 50000 l­îng qu©n phÝ cho Thiªn V­¬ng bang cã thÓ quay l¹i m«n ph¸i. ")
	Msg2Player("Hæ trî 50000 l­îng qu©n phÝ cho Thiªn V­¬ng bang cã thÓ quay l¹i m«n ph¸i. ")
end

function return_no()
	Talk(1,"","Ta chØ lµ nh©n tiÖn ghÐ th¨m ®ång m«n th«i!")
end
...
function L60_get_yes()
	Talk(1,"","<color=Red>'Thiªn V­¬ng Di th­'<color> ®­îc giÊu trong mét <color=Red>B¶o r­¬ng <color>, B¶o r­¬ng ®­îc giÊu ë <color=Red>phÝa b¾c Thanh Loa ®¶o<color>, tr­íc tiªn cÇn ph¶i t×m ®­îc <color=Red>Ch×a khãa ®Ó më B¶o r­¬ng<color>. Ng­¬i h·y lËp tøc ®i thuyÒn ®Õn <color=Red>Thanh Loa ®¶o<color>. Nhí pahØ hÕt søc cÈn thËn!")
	SetTask(3, 60*256+20)
