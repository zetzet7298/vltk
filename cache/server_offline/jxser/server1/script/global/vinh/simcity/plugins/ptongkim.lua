...
function SimCityTongKim:OnDeath(nNpcIndex, currank, attackerIndex)
	...
	BT_SetData(PL_KILLNPC, BT_GetData(PL_KILLNPC) + 1) --¼ÇÂ¼Íæ¼ÒÉ±ÆäËüÍæ¼ÒµÄ×ÜÊý
	...
	local earnbonus = 0
	earnbonus = (POINT_TK * rankradio) + npoint_lientram
	pointplayer = bt_addtotalpoint(earnbonus) or 0
	mar_addmissionpoint(earnbonus)
	-- Msg2Player("B¹n h¹ gôc ®èi ph­¬ng nh©n vµ nhËn ®­îc <color=yellow>" ..
	-- 	pointplayer .. " ®iÓm tÝch lòy "..msg_str)
	local rankname = "";
