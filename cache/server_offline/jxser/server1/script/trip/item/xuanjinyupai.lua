--Ðþ½ðÓñÅÆÓ¦¸ÃÊÇ ¿ç·þµÀ¾ß
--²»Ó¦¸ÃºÍ¿ç·þËÎ½ðÓÐ¹Ø£¬Òò´Ë·ÅÔÚÕâÀï
--¸ÃÎÄ¼þ¶¨ÒåµÄÓÐ¹Ø¿ç·þËÎ½ðº¯ÊýÓ¦×ªÒÆÖÁ£¬\mission\battle
	...
	for i=1, getn(BattleManagerDef) do
		...
		if pManager then
			if pManager.nMinEqValue and pManager.nMaxEqValue then
				szTitle = szTitle..format("%s yªu cÇu gi¸ trÞ binh gi¸p(%d~%d)<enter>", pManager.szName, pManager.nMinEqValue, pManager.nMaxEqValue)
			elseif pManager.nMinEqValue and not pManager.nMaxEqValue then
				szTitle = szTitle..format("%s yªu cÇu gi¸ trÞ binh gi¸p %d trë lªn<enter>", pManager.szName, pManager.nMinEqValue)
			elseif pManager.nMaxEqValue and not pManager.nMinEqValue then
				szTitle = szTitle..format("%s yªu cÇu gi¸ trÞ binh gi¸p%d trë xuèng<enter>", pManager.szName, pManager.nMaxEqValue)
			else
				szTitle = szTitle..format("%s kh«ng yªu cÇu gi¸ trÞ binh gi¸p", pManager.szName)
			end
...
function select_mamager(nMapId, nServerId)
	...
	if (not pManager.nMinEqValue or nValue >= pManager.nMinEqValue) and
		...
		return  Talk(1, "", format("Gi¸ trÞ binh gi¸p cña ng­¬i(%d)kh«ng phï hîp yªu cÇu", nValue))
	end
end
