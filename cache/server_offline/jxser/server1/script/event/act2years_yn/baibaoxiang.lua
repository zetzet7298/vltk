...
function BaiBaoXiang_Get(nItemCount)
	if CheckStoreBoxState(2) > 0 then
		Say("ThÇn bÝ th­¬ng nh©n: §¹i hiÖp ®· cã B¸ch b¶o r­¬ng råi, cÇn g× ®Õn ta n÷a.",1,"§ång ý!/no");
		return 1;
	end
	if	CheckStoreBoxState(1) == 0 then
		Say("ThÇn bÝ th­¬ng nh©n: §¹i hiÖp vÉn ch­a më r­¬ng thø hai, ta kh«ng thÓ trao ®æi B¸ch b¶o r­¬ng ®­îc. H·y ®Õn Ba L¨ng HuyÖn më r­¬ng thø hai tr­íc råi h·y t×m ta.",1,"§ång ý!/no");
		return 1;	
	end
	...
	for i = 1, nItemCount do
		...
		if (TB_Material[szItemKey] == nil) then
				Say("ThÇn bÝ th­¬ng nh©n: Kh«ng ®­îc råi, ta chØ cÇn 1 bé tµng b¶o ®å vµ 1 con ngùa cÊp 80, kh«ng cÇn nh÷ng thø kh¸c.",2,"Ha ha, ®Ó ta thö l¹i vËy./BaiBaoXiang_Give_UI","ThËt ng¹i qu¸, ta sÏ quay l¹i sau./no");
				return
		end
		...
		for szkey, tb_item in TB_Material do
				if (tb_enhanceitem_count[szkey][1] ~= 1) then
					Say("ThÇn bÝ th­¬ng nh©n: Kh«ng ®­îc råi, vËt phÈm ta yªu cÇu kh«ng vÉn ch­a ®ñ.",2,"Ha ha, ®Ó ta thö l¹i vËy./BaiBaoXiang_Give_UI","ThËt ng¹i qu¸, ta sÏ quay l¹i sau./no");
				return
				end
		end
		...
		Say("ThÇn bÝ th­¬ng nh©n: Xin chóc mõng, ®¹i hiÖp ®· nhËn ®­îc B¸ch b¶o r­¬ng.",1,"§ång ý!/no");
		OpenStoreBox(2);
