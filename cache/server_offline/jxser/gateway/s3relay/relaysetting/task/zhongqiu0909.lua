...
function TaskShedule()
	...
	local szMsg = format("=====%s SE BAT DAU %d:%d VA %d PHUT SAU SE KET THUC=======", "MidA200909",nNextHour, nNextMinute, nInterval)
	OutputMsg(szMsg);
end
...
local _PublicRank = function(nId)
	...
	if szName ~= nil and szName ~= "" then
		if nId == 1 then
			szMsg = format("ChÛc mıng %s %s Æ∑ Æπt Æ≠Óc th¯ nh t, thµnh vi™n bang hÈi ÆÒ Æi“u ki÷n c„ th” nhÀn l y %u kinh nghi÷m",szName, "L«n nµy Æ¯ng hµng", 6e7)
		elseif nId == 2 then
			szMsg = format("ChÛc mıng %s %s Æ∑ Æπt Æ≠Óc th¯ nh t, thµnh vi™n bang hÈi ÆÒ Æi“u ki÷n c„ th” nhÀn l y %u kinh nghi÷m",szName, "MÁi tu«n Æ¯ng hµng", 15e7)
		elseif nId == 3 then
			szMsg = format("ChÛc mıng %s %s Æ∑ Æπt Æ≠Óc th¯ nh t, thµnh vi™n bang hÈi ÆÒ Æi“u ki÷n c„ th” nhÀn l y %u kinh nghi÷m",szName, "Toµn bÈ hoπt ÆÈng Æ¯ng hµng", 20e7)
		end
	end
...
function TaskContent()
	...
	if nTime >= 2000 and nTime < 2030 then
		local szMsg = ""
		if ( nWeek == 1 or nWeek == 2 or nWeek == 3 or nWeek == 4 or nWeek == 5 or nWeek == 6 or nWeek == 0)  then
			%_PublicRank(1)
		end
		if nWeek == 0 then
			%_PublicRank(2)
		end
		if nDate == 190124 then
	...
	if (nTime >= 1930 and nTime < 2000 or nTime >= 1830 and nTime < 1925 or nTime >= 1730 and nTime < 1725 or nTime >= 1630 and nTime < 1625) and ( nWeek == 1 or nWeek == 2 or nWeek == 3 or nWeek == 4 or nWeek == 5 or nWeek == 6 or nWeek == 0) then
		callnpc_flag = 1	
